...
2. 确认只有当前 7 天窗口可解析时，主窗口才显示 Codex 额度百分比；界面不再展示已取消的 5 小时额度。
...
1. 完成至少两次间隔超过 1 分钟的成功刷新，确认 24 小时看板显示 7 天额度消耗卡片。
2. 确认柱形展示检测到的额度下降百分点，当前剩余额度单独显示。
3. 确认额度重置或恢复不会产生负消耗，长时间刷新断档不会被归入柱形。
2. 切换 7 天和 30 天范围，确认不会影响悬浮 widget，也不会触发网络请求。
3. 确认额度重置不会被计入消耗，界面明确描述为百分比变化，而不是 Token 用量。
4. 检查本地历史文件，确认其中不包含邮箱、account id、凭据或原始接口响应。
...
3. 授权后确认账号区域显示登录邮箱，刷新后显示当前 Codex 7 天额度。
