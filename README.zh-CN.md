...
> 当前状态：`0.1.74` 为 macOS 早期预览版。本版本仅支持 Codex OAuth；已提供 Windows 便携预览包，可用于 Windows 10/11 测试。
...
从[最新发布页](https://github.com/whnnick/readycheck/releases/latest)下载 `ReadyCheck-0.1.74-macos.dmg`，打开 DMG 后将 `ReadyCheck.app` 拖入“应用程序”。

Windows 10/11 预览测试可从同一个发布页下载 `ReadyCheck-0.1.74-windows-x64-portable.zip`，解压后运行 `ReadyCheck.exe`。
...
DMG 输出到 `dist/ReadyCheck-0.1.74-macos.dmg`。
...
- 上游用量响应属于内部服务接口，可能变化。ReadyCheck 只有在解析器能够验证对应字段时才显示额度百分比和 Credits 信息。
